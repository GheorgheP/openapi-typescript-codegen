...
function getOperationParameter$1(openApi, parameter) {
    ...
    if (parameter.type === 'array' && parameter.items) {
        const items = getType$1(parameter.items.type);
    ...
    if (parameter.type) {
        const definitionType = getType$1(parameter.type);
...
function getOperationResponse$1(openApi, response, responseCode) {
    ...
    if (response.schema) {
        if (response.schema.$ref) {
            const model = getType$1(response.schema.$ref);
            ...
            const model = getModel$1(openApi, response.schema);
...
function getContent(openApi, content) {
    ...
    return (content['application/json-patch+json'] &&
        content['application/json-patch+json'].schema) || (content['application/ld+json'] &&
        content['application/ld+json'].schema) || (content['application/json'] &&
